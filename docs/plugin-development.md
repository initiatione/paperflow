...
Paper Source 是 `plugins/paper-source` 的 display name；当前插件 machine-facing name 是 `paper-source`。`epi` 只作为 pre-Stage-2 legacy alias 和内部 artifact/schema 兼容词保留。

Paper Wiki 是 `plugins/paper-wiki` 的 display name；当前插件 machine-facing name 是 `paper-wiki`。`prw` 只作为 pre-Stage-2 legacy alias 和内部 artifact/schema 兼容词保留。
...
- `paper-source-paper-deposition`：Paper Source compatibility adapter，不是用户级正式写页主入口。
- Paper Wiki 不初始化、修复、reset 或静默创建 vault 结构；缺 `_paper_source/`、`_meta/`、`.obsidian`、`.git` 或正式页面根目录时，指回 Paper Source `wiki-setup`。legacy `_epi/` 只作为旧仓库输入读取。
