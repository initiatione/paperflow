...
- For complex requests, prefer `--agent-query-plan-json` or repeated `--query-variant` / `--domain-focus-term`; use `--year-min` for explicit recency windows, `--code-policy prefer|require` for public-code requests, and `--selection-policy` for ranking-to-staging behavior. Include synonyms/acronym expansions in agent-plan `synonyms`, `synonym_terms`, `acronym_expansions`, `acronyms`, `related_terms`, or `expanded_terms`; they remain soft recall unless supplied as `hard_domain_anchors` / `hard_constraints`. Agent-plan `domain_focus_terms` are legacy soft recall unless supplied as `hard_domain_anchors` / `hard_constraints`.
- Use `research_mode`, `paper_classification`, `ranking_rubric`, and EasyScholar `未核实` states as explanation contracts.
- Before ranking or recommending, let `discover-papers` / `dry-run` deduplicate against the target vault's `_meta/reference-index.json` as the canonical lightweight backlog. Only use `_paper_source\raw\*\metadata.json` as fallback when that index is missing or unreadable.
- When Zotero is enabled/configured, discovery performs read-only whole-library Zotero dedupe after wiki/raw-library checks and reports `already_in_zotero_not_wiki`, `possible_zotero_duplicate`, `new_candidate`, `zotero_unavailable`, or `not_checked` in `zotero-dedupe-record.json` / `session_recommendations.zotero_dedupe`. Do not treat this as Paper Source Zotero sync/apply.
