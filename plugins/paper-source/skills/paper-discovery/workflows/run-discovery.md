...
Use `--json` when another agent/script needs the run id and artifact paths. `discover-papers --json` returns the high-level run id, underlying discovery run id, optional auto-staging run id, `auto_staging_plan`, status counts, and artifact paths. `dry-run --json` includes progress artifact paths (`progress-events.jsonl`, `progress-summary.json`) while human progress lines go to stderr so stdout remains parseable JSON. The default broad keyword source list excludes Unpaywall because it is a DOI lookup / OA locator; configure `PAPER_SEARCH_MCP_UNPAYWALL_EMAIL` / `UNPAYWALL_EMAIL` before judging DOI-backed PDF recall or exact DOI lookup. Use `--no-query-plan` only for debugging or exact narrow topics.
...
Before reporting success, inspect `query-plan.json`, `search-record.json`, `filter-report.json`, `discovery-diagnostics.json`, `rank.json`, `_paper_source/runs/<run-id>/report.md`, `report.json`, `run-state.json`, `progress-events.jsonl`, `progress-summary.json`, review `state.json`, `candidates.json`, `shortlist.json`, `fetch_plan.json`, and `coverage.json`. Track `paper_type`, `classification_confidence`, `ranking_confidence`, hard/soft query provenance, phase timings / last-known phase from `discovery_progress`, `needs_pdf`, plus per-paper `acquire_failed`, `parse_failed`, or `prepare_failed` if source-staging later ran.
