...
Zotero 是可选 side tool，不是论文/wikibase 质量门。Paper Source 有两个不同边界：一是 discovery-time read-only dedupe，二是历史兼容的 record-only sidecar。Discovery dedupe 在 `_meta/reference-index.json`/raw-library 检查之后运行，通过官方 Zotero helper adapter 只读调用 `status`、`inventory`、必要的 `search`，把候选标成 `already_in_zotero_not_wiki`、`possible_zotero_duplicate`、`new_candidate`、`zotero_unavailable` 或 `not_checked`，并写 `zotero-dedupe-record.json` / `session_recommendations.zotero_dedupe`；它不 import Zotero，不写 formal page，不刷新 `_meta/reference-index.json` 或 `references.bib`。`record-wiki-ingest` 会根据 `<vault>/_paper_source/meta/paper-source-config.yaml` 的 `zotero.enabled` / `zotero.collection` 写本地 `_paper_source/raw/<slug>/zotero-record.json`：禁用或未配置时记录 `status=skipped` 和 reason；启用时记录 `status=recorded`、collection、metadata snapshot、`wiki-ingest-record.json` 摘要和 final wiki page hashes。record-only sidecar 不调用外部 Zotero API、不删除已有 Zotero 数据；`report.md` / `report.json` 会显示 `zotero_results`，让 Step 12 -> Step 14 在运行证据里闭合。正式 Zotero sync/status/apply 和任何 Zotero 写入属于 Paper Wiki。
