...
Paper Wiki writes the request artifact; Paper Source consumes it：`paper-wiki-record-request.json` 使用 `paper-wiki-record-request-v1` 和 `automation_mode=ask`，携带最终页路径/hash、`final-source-review.json` 路径/hash 与 human approval identity。Legacy `prw-record-request.json`、`prw-record-request-v1` 和 `--from-prw-request` remain accepted only for existing artifacts。它是 agent-mediated 完成态记录器，不替 final wiki agent 写页面。
