...
当前 `plugin.json` 中的版本：Paper Source `2.9.1`，Paper Wiki `1.3.1`。本轮完成 Zotero collaboration docs-release 对齐（详见 CHANGELOG Paper Source 2.9.1 / Paper Wiki 1.3.1）：版本镜像、manifest 前缀、Paper Source Zotero sidecar UI 文案、progress docs 和 contract tests 都指向最终职责分工；Paper Source 只拥有 discovery-time read-only Zotero dedupe 与 record-only sidecar compatibility，Paper Wiki 拥有 Zotero status/dry-run/apply 写入。上一轮新增 Paper Source discovery-time Zotero dedupe（详见 CHANGELOG Paper Source 2.9.0）：Paper Source 先用 wiki `_meta/reference-index.json` 去重，再在 Zotero 启用/配置时通过官方 helper adapter 只读检查 whole-library top-level bibliographic items，把候选标为 `already_in_zotero_not_wiki`、`possible_zotero_duplicate`、`new_candidate`、`zotero_unavailable` 或 `not_checked`，写 `zotero-dedupe-record.json`，并在 `session_recommendations.zotero_dedupe` / report tables 中单独展示 Zotero-only 和 possible duplicate 候选；不 import Zotero、不写正式 wiki、不刷新 `_meta/reference-index.json`、不写 `references.bib` 或 Paper Wiki sync report。上一轮新增 Paper Wiki Zotero apply（详见 CHANGELOG Paper Wiki 1.3.0）：Paper Wiki 可在 one-run plan-hash approval 后执行 Zotero 同步写入，link existing item、import wiki-only DOI/arXiv 论文、要求 Zotero selected target 为固定 `Paper Wiki` collection、写 nested `zotero:` metadata、按需补充/验证 bibliographic identity、只对覆盖非空 bibliographic 字段的页面创建单页 snapshot、刷新 `_meta/reference-index.json` 和 wiki-scoped `references.bib`，并写 `_meta/zotero-sync` run/index report。上一轮新增 Paper Wiki Zotero status/dry-run（详见 CHANGELOG Paper Wiki 1.2.0）：Paper Wiki 通过 `$paper-research-wiki` 拥有只读 Zotero 状态/同步预览入口，扫描 formal `references/*.md`、验证已存在 `zotero.item_key`、规划 `link_existing` / `ready_for_import` / `needs_metadata_supplementation` / conflict outcome、把 Zotero-only whole-library 文献按 summary + top 20 输出；默认不写正式页、`_meta/reference-index.json`、`references.bib`、Zotero、Paper Source sidecar 或 sync report。上一轮新增 Paper Wiki Zotero foundation（详见 CHANGELOG Paper Wiki 1.1.0）：Paper Wiki 拥有 plugin-local nested `zotero:` frontmatter、reference-index 可选 Zotero 对象、sync report schema 和 wiki-scoped BibTeX 组合 helper。上一轮新增官方 Zotero helper adapter 合同（详见 CHANGELOG 2.8.2 / 1.0.6）：Paper Source 和 Paper Wiki 各自拥有 plugin-local thin adapter，通过 trusted helper discovery、identity/command validation、bounded subprocess execution、stable gate normalization 和 redacted diagnostics 调用官方 Zotero plugin helper，不复制、不 fork、不直接实现 Zotero local API。上一轮修复 Obsidian 关系图谱维护（详见 CHANGELOG 2.8.1 / 1.0.5）：Paper Source `wiki-setup` 不再把正式页范围写成 `.obsidian/graph.json` search regex，而是保持 `search` 为空并通过 `.obsidian/app.json` `userIgnoreFilters` 隐藏内部路径；Paper Wiki 把“关系图谱出错 / 图谱只剩 index”纳入路由和 workflow，先区分 display/config drift、content graph drift、runtime/cache drift，再分别修复或提示重开 Graph tab / reload Obsidian。上一轮新增 `health-doctor`（详见 CHANGELOG 2.8.0）：把 Paper Source、Paper Wiki、MCP、runtime.json/env files、OpenAI-compatible/Grok gateway、Cloudflare 403/non-JSON upstream、MinerU、EasyScholar、Zotero 和 vault contract 健康检查收拢为只读优先的总分诊 skill。历史条目和完整变更链见 `docs/CHANGELOG.md`；runtime claims 必须继续区分 source checkout、installed cache 和当前 Codex session。
...
- 配置与安装：`health-doctor`、`config-setup`、`doctor --json`、`runtime.json`、provider readiness、`MINERU_TOKEN` set/missing 报告、paper-search MCP/CLI fallback、adaptive Python detection、`import paper_search_mcp` 探测、OpenAI-compatible/Grok gateway 分类和脱敏 MCP/env 探针。
- 论文发现：`discover-papers` 自然语言默认入口、用户画像/config 驱动的 query planner、agent-supplied `--query-variant` / `--domain-focus-term` / `--agent-query-plan-json`、required concept groups、runtime progress telemetry、wiki `_meta/reference-index.json` first dedupe、hard/soft query-plan provenance、request constraints (`--year-min`, `--code-policy`)、selection policies、`progress-events.jsonl` / `progress-summary.json`、`discovery-diagnostics.json`、`recall-gap-record.json`、`quality-risk-record.json`、`venue prior`、`domain ontology`、`two-stage retrieval`、`citation graph`、`evaluation set`、source routing、source coverage、EasyScholar default-on、review/survey 默认保留、`session_recommendations`、`auto_staging_plan` 和 `未核实` 指标。
...
- 质量环：`Plugin Eval`、`paper-source-quality-gates`、`discovery-benchmark`、benchmark、`paper-source-benchmark-v1`、`evaluation-brief`、`paper-source-improvement-brief-v1`、`source_completeness` 和 `quality_loop_sources_complete`。
...
3. 当前源码已进入 Paper Source `2.9.1` / Paper Wiki `1.3.1`；当前机器历史上已把 `paperflow` marketplace source 切到本地 `<repo-root>`，并安装过 Paper Source `1.1.0` 到 installed cache。`2.9.1` / `1.3.1` 仍需 marketplace refresh/reinstall/installed-cache verification 后才可声称运行态已更新；若要让 Git marketplace 用户获得同版本，还需要 commit/push 后再从 Git source upgrade。
...
- 当前机器历史上曾把 Paper Source installed cache 刷新到 `1.1.0`；开发源已是 `<repo-root>/plugins/paper-source` 的 `2.9.1` 和 `<repo-root>/plugins/paper-wiki` 的 `1.3.1`，用户级 `runtime.json` 不随 cache 版本目录替换。`2.9.1` / `1.3.1` 仍需 marketplace refresh/reinstall/installed-cache verification；Git marketplace 仍需 commit/push 后再 upgrade 才能让其他安装源看到同一版本。
