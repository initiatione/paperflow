...
3. 当前源码已进入 Paper Source `2.8.2` / Paper Wiki `1.1.0`；当前机器历史上已把 `paperflow` marketplace source 切到本地 `<repo-root>`，并安装过 Paper Source `1.1.0` 到 installed cache。`2.8.2` / `1.1.0` 仍需 marketplace refresh/reinstall/installed-cache verification 后才可声称运行态已更新；若要让 Git marketplace 用户获得同版本，还需要 commit/push 后再从 Git source upgrade。
...
- 当前机器历史上曾把 Paper Source installed cache 刷新到 `1.1.0`；开发源已是 `<repo-root>/plugins/paper-source` 的 `2.8.2` 和 `<repo-root>/plugins/paper-wiki` 的 `1.1.0`，用户级 `runtime.json` 不随 cache 版本目录替换。`2.8.2` / `1.1.0` 仍需 marketplace refresh/reinstall/installed-cache verification；Git marketplace 仍需 commit/push 后再 upgrade 才能让其他安装源看到同一版本。
