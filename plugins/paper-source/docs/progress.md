...
当前 `plugin.json` 中的版本：Paper Source `2.5.3`，Paper Wiki `1.0.4`。本轮是 installed Grok MCP patch（详见 CHANGELOG 2.5.3）：插件 `.mcp.json` 除 `paper-search-mcp` 外也自注册 `grok-search-rs`，外层 launcher 从用户级 runtime.json 读取 `grok_search_mcp.command` / `args` / env files 后启动用户配置的 Grok stdio MCP；Paper Source discovery 内部仍保留 Grok adapter 的 targeted/parallel gate、模型 fallback、bounded retry、质量诊断和 `grok-search-*` artifacts。上一轮是 Grok supplemental diagnostics patch（详见 CHANGELOG 2.5.2）：Grok no-contribution run 会记录 failure stage、retry attempts、returned/usable/evidence-only/quarantined/merged/filtered/ranked/accepted counts。再上一轮是 discovery evaluation gates minor bump（详见 CHANGELOG 2.5.0）：新增 `discovery-benchmark` CLI，可读取 `paper-source-discovery-benchmark-cases-v1` 本地 fixture case，复用 query-plan、normalize/filter、recall/risk、rank 和 session recommendation helper，输出 `paper-source-benchmark-v1` 供 `evaluation-brief` / Plugin Eval 证据链使用；首批 gate 覆盖 precision、recall、review leakage、duplicate rate、verified metric coverage、normalized citation ordering、absolute citation display 和 config-driven query-plan regressions。历史条目和完整变更链见 `docs/CHANGELOG.md`；runtime claims 必须继续区分 source checkout、installed cache 和当前 Codex session。
