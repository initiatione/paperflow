...
当前 `plugin.json` 中的版本：Paper Source `2.4.0`，Paper Wiki `1.0.4`。本轮是 quality-risk / recall-gap discovery minor bump（详见 CHANGELOG 2.4.0）：dry-run 新增 `recall-gap-record.json` / `paper-source-recall-gap-record-v1`，从 provider metadata 中的 official/journal version、related papers、cited-by 和 references 恢复 missed candidates，并用同一 hard filters 后标记 `paper-source-recall-expansion-v1`；同时新增 `quality-risk-record.json` / `paper-source-quality-risk-record-v1`，把 retraction、withdrawal、expression-of-concern、paper-mill、predatory-venue 等 provider 风险信号规范成 `quality_risk`，缺失风险数据明确为 `unverified`，verified severe 风险通过 `quality_gate.dimensions.quality_risk`、`verified_quality_risk` blocker 和 session recommendation cautions 降低推荐资格。上一轮是 query-planning relevance diagnostics patch（详见 CHANGELOG 2.3.14）：`query-plan.json` 写 `term_provenance_detail` 和 `paper-source-query-plan-diagnostics-v1`，agent query plan 可提供 field-agnostic synonyms/acronym/related recall terms，filter/rank 相关性使用 token-boundary lexical matching。再上一轮是 discovery ranking signal calibration patch（详见 CHANGELOG 2.3.13）：`rank_candidates` 使用 publication-age-normalized citation quality，configured venue prior 不能单独推出 Tier A，freshness 相对 current-year / explicit `year_min`，method rigor、validation strength、benchmark evidence、reproducibility 和 code/data availability 进入 score、tier 与 rationale。历史条目和完整变更链见 `docs/CHANGELOG.md`；runtime claims 必须继续区分 source checkout、installed cache 和当前 Codex session。
