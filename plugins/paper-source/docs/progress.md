...
当前 `plugin.json` 中的版本：Paper Source `2.5.0`，Paper Wiki `1.0.4`。本轮是 discovery evaluation gates minor bump（详见 CHANGELOG 2.5.0）：新增 `discovery-benchmark` CLI，可读取 `paper-source-discovery-benchmark-cases-v1` 本地 fixture case，复用 query-plan、normalize/filter、recall/risk、rank 和 session recommendation helper，输出 `paper-source-benchmark-v1` 供 `evaluation-brief` / Plugin Eval 证据链使用；首批 gate 覆盖 precision、recall、review leakage、duplicate rate、verified metric coverage、normalized citation ordering、absolute citation display 和 config-driven query-plan regressions。上一轮是 quality-risk / recall-gap discovery minor bump（详见 CHANGELOG 2.4.0）：dry-run 新增 `recall-gap-record.json` / `paper-source-recall-gap-record-v1`，从 provider metadata 中的 official/journal version、related papers、cited-by 和 references 恢复 missed candidates，并用同一 hard filters 后标记 `paper-source-recall-expansion-v1`；同时新增 `quality-risk-record.json` / `paper-source-quality-risk-record-v1`，把 retraction、withdrawal、expression-of-concern、paper-mill、predatory-venue 等 provider 风险信号规范成 `quality_risk`，缺失风险数据明确为 `unverified`，verified severe 风险通过 `quality_gate.dimensions.quality_risk`、`verified_quality_risk` blocker 和 session recommendation cautions 降低推荐资格。再上一轮是 query-planning relevance diagnostics patch（详见 CHANGELOG 2.3.14）：`query-plan.json` 写 `term_provenance_detail` 和 `paper-source-query-plan-diagnostics-v1`，agent query plan 可提供 field-agnostic synonyms/acronym/related recall terms，filter/rank 相关性使用 token-boundary lexical matching。历史条目和完整变更链见 `docs/CHANGELOG.md`；runtime claims 必须继续区分 source checkout、installed cache 和当前 Codex session。
