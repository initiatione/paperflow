...
- `wiki_query.py` 同时保留 legacy manifest/index 视角的 `wiki-query`，并提供 read-only `wiki-ask` formal graph retrieval：读取正式页根目录，扩展 backlinks/outlinks/aliases/tags/co-links，输出具体的证据/综合/推断/不确定 answer sections 和 correction candidates；它优先只读解析正文 `## 原文与证据入口` 中的 `_paper_source/raw/<slug>/paper.pdf` PDF URI 来确认源工件可用性，旧 frontmatter `sources` 或内部 wikilink PDF 只作为兼容读取和修复候选，但 `_paper_source/raw` 不作为 formal graph node；它不写 `log.md`、正式页、QMD、`paper-wiki-record-request.json` 或 Paper Source artifacts。
