...
- `grok_search_policy.py` 和 `grok_search_adapter.py` 负责可选 Grok supplemental discovery：policy 解析 targeted/parallel/off、good-enough gate、query budgets、parallel shortfall queries、可配置 parallel fan-in grace 和 Grok-only recommendation quota；adapter 调用本地 `grok-search-rs MCP` 的 `web_search`，按 `OPENAI_COMPATIBLE_MODEL` + `PAPER_SOURCE_GROK_MODEL_FALLBACKS` / 默认 multi-agent 优先列表处理 provider fallback 与 `source_fallback` 重试，只把 DOI/arXiv/publisher landing/PDF/Semantic/OpenAlex/CrossRef-like 或强论文元数据结果标准化为候选，普通网页写入 `grok-search-evidence.json`，并在 raw artifact 中记录非 secret 的 `model_attempts`。
...
- `rank_papers.py` 负责 paper type classification、ranking signals、ranking rubric、quality gate dimensions、ranking protocol、selection policies 和三角色排序解释。`priority_keywords` 是严格 topic-fit 通道；没有 priority anchors 时，positive keyword topic fit 使用饱和命中规则，`keyword_coverage_score` 仅作为长 profile 诊断。
