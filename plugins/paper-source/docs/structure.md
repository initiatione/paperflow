...
- `doctor.py` 负责只读健康检查；外部依赖缺失是 warning，插件结构缺失才 error。它还报告 `mcp_outer_launcher` 和 `codex_mcp_registration`，用于区分插件 `.mcp.json` 外层 `paper-search-mcp` / `grok-search-rs` bootstrap 不可启动、`cwd` / 相对 launcher 配置错误、未展开的 `${CLAUDE_PLUGIN_ROOT}` / `${PLUGIN_ROOT}` 占位符、用户级 `config.toml` 静态 MCP 覆盖、runtime.json 内层解释器或 Grok command 失效这几层问题。
- `runtime_config.py` 负责从 `%USERPROFILE%\.codex\plugins\paperflow\paper-source\runtime.json` 加载本机工具 runtime 配置，只补缺失的进程环境变量，不覆盖显式 env，不保存 token 明文；paper-search provider key/email 只能通过 `paper_search_mcp.env_file` 或显式进程环境进入，Grok provider key 只能通过 `grok_search_mcp.env_file` / `grok_search_mcp.env_files` 或显式进程环境进入。runtime env file 默认必须放在用户级插件 runtime 目录下，不能依赖开发源码 checkout、项目 `.env` 目录、vault 内部 `_paper_source`、临时目录或版本化 plugin cache；`runtime_config.py` 会生成 `path_policy`，`doctor.py` 会用 `runtime_path_policy` 暴露路径越界、开发 checkout 依赖和 `.env` helper command 等问题。
- `paper_search_adapter.py` 优先调用外部 `paper-search-mcp` stdio server，并在失败、超时或空结果时回退到 `paper-search` CLI，把上游输出标准化为 Paper Source 候选记录；upstream paper-search-mcp 是 broad retrieval / metadata / dedupe / download/read capability / source coverage provider，不是 semantic quality gate 或 recommendation ranker，Paper Source 自己负责 query planning、source routing、DOI policy、quality gates、ranking 和 `session_recommendations`；`plan_source_routing` 会把 Google Scholar 等 `unstable` source 默认降级，把 Unpaywall 这类 `doi-lookup` source 从广义关键词首轮检索中降级，只在 DOI 精确查询和 OA 补链阶段使用，并把 Unpaywall email 缺失写成 `source_routing.provider_gaps` / `unpaywall_email_missing`；dry-run report 会保留 `source_coverage`，包含 `sources_used`、`source_results`、`errors`、`raw_total`、`deduped_total`、`query_count`、source capability matrix、provider readiness、source routing 和 provider gaps；`doctor --json` 也会输出 `paper_search_provider_readiness`，暴露 Unpaywall/CORE/Semantic Scholar/Google Scholar/DOAJ/Zenodo provider env 缺口；acquire 后可调用 MCP `read_<source>_paper` 或 CLI read 写 non-authoritative retrieval preview。
- `grok_search_policy.py` 和 `grok_search_adapter.py` 负责可选 Grok supplemental discovery：policy 解析 targeted/parallel/off、good-enough gate、query budgets、parallel shortfall queries、可配置 parallel fan-in grace 和 Grok-only recommendation quota；插件 `.mcp.json` 同时通过 `scripts/grok_search_mcp_launcher.*` 把 `grok-search-rs` 暴露为安装后可见的 MCP server，但 discovery workflow 仍由 adapter 负责调用本地 `grok-search-rs MCP` 的 `web_search`，按 `OPENAI_COMPATIBLE_MODEL` + `PAPER_SOURCE_GROK_MODEL_FALLBACKS` / 默认 multi-agent 优先列表处理 provider fallback 与 `source_fallback` 重试，只把 DOI/arXiv/publisher landing/PDF/Semantic/OpenAlex/CrossRef-like 或强论文元数据结果标准化为候选，普通网页写入 `grok-search-evidence.json`，并在 raw artifact 中记录非 secret 的 `model_attempts`。
- `paper_library.py` 负责轻量文献去重索引：优先读取 wiki `_meta/reference-index.json`，按 DOI、arXiv base id、source_id、normalized title 建键；该文件加载成功时就是唯一 canonical backlog，不再额外扫描 raw。只有 reference-index 缺失/不可用时，才扫描 `_paper_source/raw/*/metadata.json` 作为 fallback。命中正式 wiki 页时 filter reason 是 `already_in_wiki:<page>`，命中 raw source bundle fallback 时是 `already_in_library:<slug>`。
...
- `recommendation_output.py` 负责 `report.json.session_recommendations` / `paper-source-session-recommendations-v1` 的 chat-facing 推荐投影；primary/review 输出现在要求 DOI-present，补不到 DOI 的候选只进入 `doi_recovery_summary`、`doi_resolution_summary` 和 `doi_filtered_summary` / `missing_required_doi` 统计，不进入推荐或待复核清单，但会话层必须显示失败清单以便人工/再次检索补 DOI；Zotero discovery dedupe 通过 `session_recommendations.zotero_dedupe` 暴露 `already_in_zotero_not_wiki`、`possible_zotero_duplicate`、`new_candidate`、`zotero_unavailable` 和 `not_checked`，其中 Zotero-only/possible duplicate 不进入 primary recommendations；`auto_staging.py` 负责 `paper-source-auto-staging-plan-v1`，从 primary recommendations 中选择最多 3 篇 PDF-available 论文进入 fast source-staging，并把 `needs_pdf` / review-sideline / execution state 写回 `auto_staging_status`；`report_run.py` 负责生成 `_paper_source/runs/<run-id>/report.md` 和 `report.json`，并提供 `report --run-id` 的只读 loader；该 CLI 会展示 run report artifact 和 `run-state.json` payload，但不刷新 index、queue、raw、staging、Zotero 或 wiki。
...
- `wiki_contracts.py` 固定七类正式页面、brief-first required Paper Source/Paper Wiki skills、frontmatter validation mirror、quality gates、retired handoff cleanup 和 QMD collection boundary 等跨模块常量；human-readable canonical contract 仍是 Paper Wiki `rules/wiki-writing-standard.md`；`paper-research-wiki` qmd collection 允许正式页目录加 `AGENTS.md`、`index.md`、`hot.md`、`log.md`、`_meta/`，必须 ignore `_paper_source/**`、`.obsidian/**`、`.claude/**`，让 `_paper_source/meta/formal-page-snapshots/`、raw MinerU source Markdown 和 staging handoff 不进入 QMD。external wiki skills are optional helpers / policy references。
...
- `graph_visibility.py` 负责 Obsidian 图谱显示配置修复：`.obsidian/graph.json` 的全局 `search` 必须保持为空，`collapse-filter` 归一化，`.obsidian/app.json` 合并内部目录 `userIgnoreFilters`。正式页范围不要再写成 graph search regex；范围边界由内部目录 ignore filters 和 Paper Wiki 的 formal wikilink/content scan 共同保证，避免 Obsidian 解析漂移导致图谱只剩 `index`。
...
- `zotero_helper_adapter.py` 负责官方 Zotero plugin helper 的 trusted discovery、identity/command validation、bounded subprocess 调用、stable gate normalization 和 redacted diagnostics；它不复制官方 Zotero 插件，不直接实现 Zotero local API，也不承担 Paper Wiki sync 业务逻辑。
- `zotero_dedupe.py` 负责 Paper Source discovery-time read-only Zotero dedupe：先保留 `_meta/reference-index.json` / raw-library 判断，再通过官方 helper adapter 只读检查 whole-library top-level bibliographic items，写 `zotero-dedupe-record.json`，把 Zotero-only 和 possible duplicate 候选从主推荐/auto-stage 路径分流到报告分组；它不 import Zotero、不写正式 wiki、不刷新 `_meta/reference-index.json` 或 `references.bib`。
...
- `zotero-sync`：历史兼容的 Zotero record-only sidecar 入口；`record-wiki-ingest` 会自动写本地 `zotero-record.json` 并把结果带入 report。Paper Source 的外部 Zotero 能力是 discovery-time dedupe；正式 Zotero sync/apply 由 Paper Wiki 拥有。
...
- Zotero record-only sidecar 只写 `_paper_source/raw/<slug>/zotero-record.json` 和 run report 中的 `zotero_results`；discovery-time Zotero dedupe 只读 helper inventory/search 并写 run 级 `zotero-dedupe-record.json` / `session_recommendations.zotero_dedupe`。两者都不得 import Zotero、改 final wiki 页、刷新 `_meta/reference-index.json`、写 `references.bib` 或删除 Zotero 数据。
...
用户级 runtime 配置不放在安装 cache 版本目录，而放在 `%USERPROFILE%\.codex\plugins\paperflow\paper-source\runtime.json`，用于保存 MCP/CLI/MinerU 命令路径、`mineru.env` 路径，以及 `paper_search_mcp.env_file` / `grok_search_mcp.env_file` / `easyscholar.env_file` 这类 secret env file 路径；插件升级 cache 不应覆盖它。
...
完整 page-family/frontmatter 契约见 Paper Wiki `plugins/paper-wiki/rules/wiki-writing-standard.md`（canonical）；Paper Source 这里只保留 validation mirror summary，用于 handoff/record gate 校验。

正式写入所需 Paper Source/Paper Wiki skill 边界是 `paper-wiki` 插件包提供的 `$paper-research-wiki`；Paper Source `paper-source-paper-deposition` 只用于历史 handoff cleanup。external wiki skills are optional helpers / policy references, including `llm-wiki`、`wiki-ingest`、`wiki-context-pack`、`wiki-lint`、`wiki-stage-commit`、`wiki-status`、`wiki-query`、`wiki-provenance`、`tag-taxonomy`。作为 Paper Source validation mirror，正式页 frontmatter 至少包含 `title`、`category`、`page_family`、`tags`、`aliases`、`sources`、`summary`、`provenance`、`base_confidence`、`lifecycle`、`lifecycle_changed`、`tier`、`created`、`updated`。`category` 和 `page_family` 要匹配目录；初始状态只能是 `draft`，旧 `review-needed` 只作为迁移/修复对象，不能使用 `source-reviewed` 或 `verified` 作为正式页 lifecycle。
