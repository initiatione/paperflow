...
- `grok_search_policy.py` 和 `grok_search_adapter.py` 负责可选 Grok supplemental discovery：policy 解析 targeted/parallel/off、good-enough gate、query budgets、parallel shortfall queries 和 Grok-only recommendation quota；adapter 调用本地 `grok-search-rs MCP` 的 `web_search`，只把 DOI/arXiv/publisher landing/PDF/Semantic/OpenAlex/CrossRef-like 或强论文元数据结果标准化为候选，普通网页写入 `grok-search-evidence.json`。
