...
- `zotero_helper_adapter.py` 负责官方 Zotero plugin helper 的 trusted discovery、identity/command validation、bounded subprocess 调用、stable gate normalization 和 redacted diagnostics；它不复制官方 Zotero 插件，不直接实现 Zotero local API，也不承担 Paper Wiki sync 或 Paper Source discovery dedupe 业务逻辑。
