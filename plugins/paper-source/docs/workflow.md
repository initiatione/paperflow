...
自然语言主题不能直接当作 MCP 主检索式。Agent 应先把用户意图形成透明 query plan，生成 5-8 条短学术 query variants，再用 `--query-variant` 或 `--agent-query-plan-json` 传给 `dry-run`；需要硬过滤时用 `--domain-focus-term`、`hard_domain_anchors` 或 `hard_constraints` 传对象/领域锚点。Agent plan 可以用 `required_concept_groups` 明确声明 target/task/problem 等必须同时覆盖的概念组；每组用任意领域词表，不写死 AUV、医学或 AI。没有显式 required groups 时，Paper Source 只在“确认 hard target anchor + 当前请求命中的 config/Research Brief task term”同时存在时保守推导 `paper-source-required-concept-groups-v1`；普通 one-concept 或 soft-recall 请求保持旧召回行为。Agent plan 也可以用 `synonyms`、`synonym_terms`、`acronym_expansions`、`acronyms`、`related_terms` 或 `expanded_terms` 记录同义词、缩写展开和召回扩展；这些默认只进入 `soft_recall_terms`，除非同一 plan 另行把它们列为 confirmed hard anchors。topic 推断词只能进入 `soft_recall_terms`，不得自动升级成 hard filter。`--year-min` 表达明确的近期窗口，`--code-policy prefer` 表达“尽可能有公开代码”，`--code-policy require` 表达硬性代码要求，`--selection-policy` 控制推荐到推进的阈值。脚本负责验证、记录和执行这些显式输入，而不是在 Python 里写死每个学科的语义词典。

`discover-papers` is the natural-language default. It writes a high-level `discover-papers` run that links the underlying discovery run and optional auto-staging run, surfaces `session_recommendations`, `auto_staging_plan`, `manual_downloads`, `discover-papers-record.json`, `report.json`, and `run-state.json`, and stops before approval/Paper Wiki. `dry-run` remains the lower-level evidence/debug command: it writes `_paper_source/runs` and resumable `_paper_source/reviews`; default resume skips provider calls for the same signature. Use `--refresh` to force provider search. Long-term library/backlog state lives in wiki `_meta/reference-index.json`, not in review sessions. It writes source coverage into `report.json.discovery_context.source_coverage`, request constraints into `report.json.discovery_context.request_constraints` / `query-plan.json`, chat-facing reading decisions into `report.json.session_recommendations`, runtime progress telemetry into `progress-events.jsonl`, `progress-summary.json`, `run-state.json.discovery_progress`, and `report.json.discovery_context.discovery_progress`, query-plan `term_provenance_detail`, `paper-source-query-plan-diagnostics-v1`, optional `paper-source-required-concept-groups-v1`, and filtering/readiness diagnostics into `discovery-diagnostics.json`, with `sources_used`, `source_results`, `errors`, `raw_total`, `deduped_total`, `query_count`, `capabilities`, `provider_readiness`, `source_routing`, `provider_gaps`, `recommendable`, `staging_ready`, `needs_pdf`, `rejected`, `required_concept_group_mismatch`, `already_in_wiki`, and `already_in_library`.

`paper-search-mcp` is the broad retrieval and metadata provider behind the default source route, not the final judge of paper quality. Paper Source treats upstream source counts, errors, rate limits, unsupported download/read capabilities, and DOI-only sources as source-coverage evidence. Paper Source owns the cross-discipline quality gate: strict `priority_keywords` drive relevance when hard anchors exist; otherwise saturated positive keyword matches can pass topic fit while `keyword_coverage_score` remains diagnostic. Filter/rank relevance uses token-boundary lexical matching with simple singular/plural handling, not raw substring matching, so short acronyms do not match inside unrelated words. `quality_gate.dimensions` explains identity, relevance, inspectability, validation, source confidence, reproducibility, request risk, and quality risk for each ranked candidate. Dry-run writes `recall-gap-record.json` for provider-supplied official/related/citation expansion evidence and `quality-risk-record.json` for verified/suspected/unverified risk metadata.

Optional `grok-search-rs MCP` is a high-quality supplemental discovery path inside this same workflow. The installed plugin exposes a `grok-search-rs` MCP server through `.mcp.json`; its outer launcher reads `grok_search_mcp.command`, `args`, and env files from user-level runtime.json, then starts the user's configured Grok stdio MCP. It is configured locally through runtime.json and controlled by vault `grok_search` plus `--grok-mode targeted|parallel|off` / `--no-grok-search`. Default targeted mode runs paper-search first and may skip Grok when paper-search is good enough. Parallel mode is aggressive shortfall recall that starts a static Grok query set beside the full paper-search lifecycle; it does not stop or shorten paper-search. The Paper Source adapter still calls MCP `web_search` with `response_format=detailed`, keeps `OPENAI_COMPATIBLE_MODEL` as the primary model, retries configured/default model fallbacks for provider fallback or `source_fallback` responses, and records non-secret `model_attempts` in `grok-search-raw.json`. When a Grok run contributes no kept papers, artifacts and reports classify whether the failure was provider/runtime, timeout/budget cutoff, source fallback, parser/normalization, page chrome, weak identity, or downstream filter/rank rejection, and show returned/usable/evidence-only/quarantined/merged/filtered/ranked/accepted counts plus retry attempts and outcomes. Empty-title, page-chrome, and regex-only DOI/arXiv records stay as evidence/quarantine unless explicit stable paper identity is present. Provider artifacts stay separate as `paper-search-record.json`, `grok-search-record.json`, `grok-search-raw.json`, and `grok-search-evidence.json` before merged `search-record.json`. Final recommendations still require at least one usable paper-search candidate; Grok-only salvage is evidence/diagnostics only.

`report.json.session_recommendations` uses `paper-source-session-recommendations-v1`: `primary_recommendations` is the capped chat-visible priority list, `review_appendix` keeps Tier C / non-Reject `review-candidate` papers out of the primary recommendation list, `existing_library_appendix` keeps already-in-wiki/raw-library papers out of new recommendations, `doi_required_policy` requires DOI-present papers for primary/review output, `doi_recovery_summary` / `doi_resolution_summary` show targeted DOI recovery success and failure counts, `doi_filtered_summary` records any remaining `missing_required_doi` candidates, `quality_reject_debug` keeps Reject records as diagnostics, `required_concept_group_rejects` keeps filter-level required-group mismatches as diagnostics, `rejected_summary` gives count/reason evidence, and `overflow.hidden_count` tells the agent when more primary candidates remain in artifacts. Chat output renders primary recommendations as a numbered list with paper title, venue/year plus DOI/citation evidence, and a required `summary：` line explaining method, target problem, and reported effect/evidence from `original_abstract` / `chinese_summary.source_text`. `existing_library_appendix` remains a separate already-in-library/wiki table, not part of the numbered recommendation list. Paper Source does not generate semantic Chinese summaries internally; the calling agent writes them from source abstract evidence.
