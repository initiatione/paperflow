...
| 层级 | 解决的问题 | 主要能力 | 关键产物 |
| --- | --- | --- | --- |
| 主题纵向层 | 最近有什么新论文、是否漏掉分支、先读哪几篇 | `topic-tracking`、`dry-run`、`research-queue` | `_meta/reference-index.json`、`_paper_source/runs/*`、coverage/backlog |
...
3. 用 `report --run-id <run-id>` 查看排序结果；持续追踪、coverage、backlog 以 `_meta/reference-index.json` 为准，`topic-tracking` 做增量视图。
...
- `dry-run` 只写 `_paper_source/runs` 和 `_paper_source/reviews` provider cache；已沉淀 backlog 以 `_meta/reference-index.json` 为准。
