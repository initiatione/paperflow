...
-> human approval -> Paper Wiki/Codex/Claude wiki ingest agent
...
| 层级 | 解决的问题 | 主要能力 | 关键产物 |
| --- | --- | --- | --- |
| 主题纵向层 | 最近有什么新论文、是否漏掉分支、先读哪几篇 | `topic-tracking`、`dry-run`、`research-queue` | `_paper_source/runs/*`、`_paper_source/reviews/*`、coverage/backlog |
...
- `dry-run` 只写 `_paper_source/runs` 和 `_paper_source/reviews`。
