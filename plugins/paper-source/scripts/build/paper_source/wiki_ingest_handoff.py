...
from paper_source.wiki_handoff_contracts import agent_context_policy as default_agent_context_policy
...
def _agent_checklist(
    *,
    ...
    checklist = [
        ...
        "Read the Paper Source evidence handoff: "
        ...
        "Batch deposition rule: compare this paper with the current batch or neighboring Paper Source source bundles before creating reusable concept or synthesis pages.",
        ...
        + ".",
        "Route formal research knowledge into the seven Paper Wiki page families when useful: "
        + ", ".join(families)
        + ".",
        "Before record-wiki-ingest, write "
        + str(final_source_review_path)
        + " using schema "
        + str(final_source_review_contract.get("record_schema_version") or "paper-source-final-source-review-v1")
        ...
        "Do not write final pages from Paper Source suggested routes directly; Paper Source must only write internal underscore artifacts.",
    ]
...
def build_wiki_ingest_handoff(vault_path: Path, slug: str) -> dict[str, Any]:
    ...
    return {
        "title": "Paper Source Wiki Ingest Handoff",
        "paper_slug": slug,
...
def render_wiki_ingest_handoff(handoff: dict[str, Any]) -> str:
    lines = [
        f"# {handoff.get('title', 'Paper Source Wiki Ingest Handoff')} - {handoff.get('paper_slug')}",
        "",
        ...
        f"paper_source_write_scope: {handoff.get('paper_source_write_scope')}",
    ...
    lines.append(f"- schema: {contract.get('record_schema_version') or 'paper-source-final-source-review-v1'}")
