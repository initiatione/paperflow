...
def _formal_page_directory_lines() -> str:
    return "\n".join(
        f"- `{path}`: final wiki page family written by the wiki skill, not by Paper Source"
        for path in FORMAL_PAGE_PATHS
    )


AGENTS_MD = """# Paper Source Paper Research Wiki
...
Paper Source writes only the internal `_paper_source/` repository. Formal graph pages are written by the wiki skill after batch deposition into `references/`, `concepts/`, `derivations/`, `experiments/`, `synthesis/`, `reports/`, and `opportunities/`. Obsidian graph views should ignore `_paper_source/`; `_paper_source/raw/<slug>/mineru/paper.md` and `_paper_source/raw/<slug>/mineru/<slug>.md` remain source material for final writing, not formal wiki pages.
...
4. agent-mediated final wiki pages written outside Paper Source internals
...
Paper Source must not create formal pages in these roots. The wiki skill creates or updates them after reading a batch of source papers. Do not invent new top-level wiki routes without vault approval.
"""
...
- `_paper_source/`: internal Paper Source repository with its own navigation, manifest, policy, and lifecycle management
- `_paper_source/raw/<slug>/`: downloaded source paper artifacts
- `_paper_source/staging/papers/<slug>/`: per-paper evidence handoff
- `_paper_source/staging/wiki-batches/<batch-id>/`: multi-paper handoff for wiki skill deposition
- `_paper_source/meta/`: Paper Source config, config history, formal-page snapshots, and bounded maintenance records
- `_paper_source/policies/retention.json`: repository cleanup policy
- `_paper_source/runs/`, `_paper_source/cache/`, `_paper_source/tmp/`, `_paper_source/tmp-manual-pdfs/`, `_paper_source/quarantine/`, `_paper_source/evolution/`: on-demand workflow directories, not empty bootstrap shells
...
These are the durable, human-readable knowledge nodes. Source paper Markdown under `_paper_source/raw` is source material for final writing, not a formal graph page.
...
Paper Source reader outputs, critic reports, staging briefs, run dashboards, and raw paper artifacts stay under `_paper_source/` and should not pollute the main knowledge graph.
    ...
    payload.update(
        {
            ...
            "paper_source_internal_root": "_paper_source",
            "paper_source_write_scope": "internal-_paper_source-repository-only",
            ...
            "operational_dirs": ["_paper_source"],
...
def initialize_paper_wiki(vault_path: Path) -> list[str]:
    ...
    created.extend(ensure_paper_source_repository(vault_path))
