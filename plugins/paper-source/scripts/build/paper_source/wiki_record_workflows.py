...
from paper_source.config import load_wiki_config
...
from paper_source.wiki_ingest_approval import create_human_approval_record, human_approval_record_path
from paper_source.wiki_ingest_record import create_wiki_ingest_record, create_wiki_ingest_record_from_paper_wiki_request
from paper_source.zotero_sync import sync_zotero_record
...
def _zotero_record_only(vault_path: Path, paper_root: Path) -> dict:
    ...
    collection = str(zotero_config.get("collection") or "Paper Source")
...
def _write_wiki_ingest_record_report(
    ...
    *,
    ...
    changed_artifacts = [
        f"_paper_source/raw/{slug}/wiki-ingest-record.json",
        f"_paper_source/staging/papers/{slug}/wiki-ingest-record.json",
    ]
    report_paper_states = [
        {
            "slug": slug,
...
def _write_human_approval_report(
    ...
    *,
    ...
    report_json_path = run_dir / "report.json"
    lines = [
        f"# Paper Source Human Approval - {slug}",
        "",
        f"- run_id: {run_id}",
    ...
    _write_json(
        report_json_path,
        {
            "schema_version": "paper-source-human-approval-report-v1",
            "run_id": run_id,
            "workflow_type": "record-human-approval",
            "paper_slug": slug,
    ...
    *,
    ...
    if from_paper_wiki_request is not None:
        ...
        if not str(slug or "").strip():
            raise ValueError("record-wiki-ingest requires --slug unless --from-paper-wiki-request is used")
        if not pages:
            raise ValueError("record-wiki-ingest requires --page unless --from-paper-wiki-request is used")
        if not str(approved_by or "").strip():
            raise ValueError("record-wiki-ingest requires --approved-by unless --from-paper-wiki-request is used")
    ...
    if source_request.get("path"):
        input_artifacts["paper-wiki-record-request.json"] = Path(str(source_request["path"]))
