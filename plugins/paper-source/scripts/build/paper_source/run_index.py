...
def _render_research_queue(queue):
    lines = ["# Paper Source Research Queue", ""]
...
def _render_dashboard(entries, summary, research_queue=None):
    ...
    lines = ["# Paper Source Run Dashboard", ""]
    if not entries:
        lines.extend(
            [
                "## Summary",
...
def refresh_run_index(vault_path):
    ...
    (runs_root / "dashboard-failures.md").write_text(
        _render_filtered_dashboard("Paper Source Failed Runs", failures, "No failed runs."),
        encoding="utf-8",
    )
    (runs_root / "dashboard-human-gate.md").write_text(
        _render_filtered_dashboard("Paper Source Human Gate Runs", human_gate_pending, "No runs waiting on a human gate."),
        encoding="utf-8",
    )
    (runs_root / "dashboard-recent-success.md").write_text(
        _render_filtered_dashboard("Paper Source Recent Successful Runs", recent_success, "No successful runs yet."),
        encoding="utf-8",
    )
...
def render_run_lifecycle(result):
    ...
    lines = [
        f"Paper Source Run Lifecycle - {action}",
        "",
...
def query_research_queue(vault_path, *, bucket=None, limit=10, include_actions=False):
    ...
    if bucket:
        ...
        return {
            "title": f"Paper Source Research Queue - {bucket}",
            "bucket": bucket,
            "items": items,
        }

    return {
        "title": "Paper Source Research Queue",
        "bucket": None,
        "queue": {
            queue_bucket: (
...
def query_runs(
    ...
    *,
    ...
    if latest_success:
        ...
        title = f"Paper Source Runs Query - latest success for {latest_success}"
    else:
        matches = runs
        title = "Paper Source Runs Query"
        if failed:
            matches = [entry for entry in matches if entry.get("status") not in _SUCCESS_STATUSES]
            title = "Paper Source Runs Query - failed runs"
        elif human_gate:
            matches = [entry for entry in matches if _human_gate_status(entry) in _HUMAN_GATE_PENDING]
            title = "Paper Source Runs Query - human gate runs"
