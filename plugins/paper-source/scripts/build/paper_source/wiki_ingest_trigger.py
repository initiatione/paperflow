...
from paper_source.wiki_ingest_approval import load_human_approval_record
from paper_source.wiki_ingest_handoff import build_wiki_ingest_handoff
from paper_source.wiki_contracts import formal_page_family_paths, page_lifecycle_states, research_review_fields


WIKI_AGENT_TRIGGER_SCHEMA_VERSION = "paper-source-wiki-agent-trigger-v1"
...
def _ready_instruction(
    *,
    ...
    return (
        ...
        "use mineru/<slug>.md as the primary source for paper text, formulas, notation, and context; "
        "use paper.pdf, metadata.json, mineru/images/*, mineru/mineru-manifest.json, figure-index.json, "
        "formula-index.json, and asset-normalization-record.json as fallback or visual-evidence checks when "
        "MinerU Markdown is missing, wrong, ambiguous, or insufficient; use optional mineru/paper.tex only "
        "as a cross-check when present and non-empty. Compare this paper with the current batch or neighboring Paper Source source "
...
def render_wiki_ingest_trigger(trigger: dict[str, Any]) -> str:
    ...
    lines = [
        f"# Paper Source Wiki Agent Trigger - {trigger.get('paper_slug')}",
        "",
