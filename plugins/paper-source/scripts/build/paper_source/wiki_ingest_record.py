...
def _validate_formal_frontmatter(relative_path: str, text: str) -> list[str]:
    ...
    if family not in formal_page_family_names():
        issues.append("formal page must be under one Paper Wiki page family: " + ", ".join(formal_page_family_names()))
...
def _validate_formal_page_shape(relative_path: str, text: str) -> None:
    ...
    if marker_hits:
        raise ValueError(
            "recorded final wiki page looks like a Paper Source audit/staging artifact: "
            + normalized
            + " markers="
            + ", ".join(marker_hits[:3])
        )
    if normalized.startswith("reports/") and normalized.endswith("-reading-report.md"):
        raise ValueError("recorded final wiki page must not be a Paper Source reading report: " + normalized)
    if normalized.startswith("concepts/") and normalized.endswith("-concept.md"):
        raise ValueError("recorded final wiki page must not be a Paper Source per-paper routes or pseudo concept page: " + normalized)
...
def _load_paper_wiki_record_request(vault_path: Path, request_path: str | Path) -> tuple[dict[str, Any], Path, str]:
    ...
    try:
        ...
        raise ValueError(f"Paper Wiki record request is invalid JSON: {resolved}: {exc}") from exc
    if not isinstance(payload, dict):
        raise ValueError(f"Paper Wiki record request must be a JSON object: {resolved}")
...
def _request_dict(payload: dict[str, Any], key: str) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise ValueError(f"Paper Wiki record request must include {key} object")
    return value
...
def _request_string(payload: dict[str, Any], key: str) -> str:
    ...
    if not value:
        raise ValueError(f"Paper Wiki record request must include {key}")
    return value
...
def _validate_paper_wiki_request_pages(vault_path: Path, payload: dict[str, Any]) -> list[str]:
    ...
    for index, item in enumerate(final_pages, start=1):
        if not isinstance(item, dict):
            raise ValueError(f"Paper Wiki record request final_pages[{index}] must be an object")
        relative_path = str(item.get("relative_path") or item.get("path") or "").replace("\\", "/").strip()
        if not relative_path:
            raise ValueError(f"Paper Wiki record request final_pages[{index}] must include relative_path")
        expected_hash = str(item.get("sha256") or "").strip()
        if not expected_hash:
            raise ValueError(f"Paper Wiki record request final_pages[{index}] must include sha256")
        page_record = _resolve_page(vault_path, relative_path)
        if page_record["relative_path"] in seen:
            raise ValueError("Paper Wiki record request duplicate final page: " + page_record["relative_path"])
        seen.add(page_record["relative_path"])
        if page_record["sha256"] != expected_hash:
            raise ValueError("Paper Wiki record request final page sha256 mismatch: " + page_record["relative_path"])
...
def _validate_paper_wiki_request_source_review(
    *,
    ...
    if not source_review_path.is_file():
        raise FileNotFoundError(f"missing Paper Wiki record request final source review: {source_review_path}")
    expected_hash = str(final_source_review.get("sha256") or "").strip()
    if not expected_hash:
        raise ValueError("Paper Wiki record request final_source_review must include sha256")
    actual_hash = file_sha256(source_review_path)
    if actual_hash != expected_hash:
        raise ValueError("Paper Wiki record request final-source-review sha256 mismatch: " + str(source_review_path))
    return source_review_path


def _paper_wiki_source_request_metadata(
    *,
    payload: dict[str, Any],
    request_path: Path,
    relative_path: str,
...
def create_wiki_ingest_record_from_paper_wiki_request(
    vault_path: Path,
    ...
    *,
    ...
    payload, resolved_request_path, relative_request_path = _load_paper_wiki_record_request(vault_path, request_path)
    ...
    if not approved_by:
        raise ValueError("Paper Wiki record request human_approval must include approved_by")
    source_request = _paper_wiki_source_request_metadata(
        payload=payload,
...
def _validate_final_source_review(
    *,
    ...
    if payload.get("schema_version") not in ACCEPTED_FINAL_SOURCE_REVIEW_SCHEMA_VERSIONS:
...
def create_wiki_ingest_record(
    ...
    *,
    ...
    if blocked_requested_routes:
        raise ValueError(
            "recorded final wiki pages must come from wiki-skill batch deposition, not Paper Source per-paper routes: "
    ...
    record_payload = {
        ...
        "record_only": True,
        "final_pages_modified_by_paper_source": False,
