...
MinerU Markdown is the primary source for formulas, notation, method context, and prose. Use the PDF, figure/formula indexes, and image evidence as fallback checks only when Markdown is missing, wrong, ambiguous, or insufficient. Optional aids are non-empty native `mineru/paper.tex` when present, reader evidence maps, claim support JSON, critic reports, and `wiki-agent-trigger.json`.
