...
`scripts/build/paper_wiki/zotero_helper_adapter.py` is the Paper Wiki-local thin adapter for the official Zotero plugin helper. It discovers trusted helper locations or explicit overrides, validates helper identity and command support, runs bounded subprocess calls, returns stable gates, and redacts diagnostics. It does not copy the official Zotero plugin, call Zotero SQLite, or implement Paper Source discovery behavior.

`scripts/build/paper_wiki/frontmatter.py`, `zotero_contract.py`, `zotero_report.py`, and `bibtex.py` provide the Paper Wiki-local Zotero data-contract foundation. `zotero_status.py` adds the read-only status/dry-run planner and human-readable renderer. Formal `references/` pages may carry optional nested top-frontmatter `zotero:` metadata; `_meta/reference-index.json` keeps schema `paper-research-reference-index-v1` and adds optional valid `zotero` objects only for formal reference pages; sync report helpers use `paper-wiki-zotero-sync-report-v1`; wiki-scoped BibTeX composition consumes item-level BibTeX supplied by adapter callers and never exports a whole Zotero library. Zotero status/dry-run defaults to no writes and reports Zotero-only whole-library items as summary plus top 20.
