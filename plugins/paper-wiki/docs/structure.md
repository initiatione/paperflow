...
`scripts/build/paper_wiki/frontmatter.py`, `zotero_contract.py`, `zotero_report.py`, and `bibtex.py` provide the Paper Wiki-local Zotero data-contract foundation. Formal `references/` pages may carry optional nested top-frontmatter `zotero:` metadata; `_meta/reference-index.json` keeps schema `paper-research-reference-index-v1` and adds optional valid `zotero` objects only for formal reference pages; sync report helpers use `paper-wiki-zotero-sync-report-v1`; wiki-scoped BibTeX composition consumes item-level BibTeX supplied by later adapter callers and never exports a whole Zotero library.
