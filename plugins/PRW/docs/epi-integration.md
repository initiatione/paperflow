...
Required inputs include `wiki-ingest-brief.json`, deprecated compatibility `wiki_deposition_task.json`, reading report, metadata, PDF, MinerU Markdown, TeX, images, and manifest. Canonical handoff = `wiki-ingest-brief.json`; `wiki_deposition_task.json` is still read but should not be the new dependency target.
