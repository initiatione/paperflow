...
- Quick check: read manifest or `.manifest.json`, `index.md`, `log.md`, `hot.md`, pending EPI handoffs, recent changed pages, and current staged pages.
...
4. QMD compatibility: whether QMD was used, whether QMD was skipped, whether `qmd update` / `qmd embed` should run, and whether fallback to manifest and file search was used.
5. What to Do Next: a ranked list of no more than six actions.
6. Completion Report when this is a post-task check: pages created or updated, links/tags/aliases repaired, tracking files updated, QMD refreshed / skipped / failed with fallback, remaining risks, and next EPI/PRW action.
