...
RAW_PAPER_PATH_PATTERN = re.compile(r"_epi/raw/(?P<slug>[^/\]\|\s)]+)/paper\.pdf", re.IGNORECASE)
RAW_PAPER_URI_PATTERN = re.compile(r"file=_epi%2Fraw%2F(?P<slug>[^%/)]+)%2Fpaper\.pdf", re.IGNORECASE)
...
def _raw_paper_path_from_source(value: str) -> str | None:
    ...
    if uri_match:
        ...
        return f"_epi/raw/{slug}/paper.pdf"
                    ...
                    corrections.append(
                        {
                            ...
                            "source": relative,
                            "target": target,
                            "message": "Formal page body links an EPI internal artifact as graph content.",
                        }
                    )
            ...
            corrections.append(
                {
                    ...
                    "source": relative,
                    "target": "sources",
                    "message": "Reference page has no frontmatter source link to _epi/raw/<slug>/paper.pdf.",
                }
            )
            continue
                ...
                corrections.append(
                    {
                        ...
                        "source": relative,
                        "target": source,
                        "message": "Frontmatter source does not point to _epi/raw/<slug>/paper.pdf.",
                    }
                )
                continue
...
def _source_evidence_phrase(page: dict) -> str:
    ...
    if not evidence:
        return "未发现可解析的 _epi/raw source evidence"
...
def _build_answer_sections(question: str, pages: list[dict], corrections: list[dict]) -> dict[str, list[str]]:
    ...
    if missing_source_pages:
        uncertainty.append(
            "部分命中页缺少可确认存在的 _epi/raw source PDF，因此 source-grounded 强度不足。"
        )
    if corrections:
    ...
    *,
    ...
    return {
        ...
        "next_actions": [
            ...
            "Ask before routing correction candidates into PRW repair/update workflows.",
        ],
    }
...
def render_wiki_ask(result: dict) -> str:
    ...
    lines.extend(
        [
            "",
            "是否需要我根据这些纠错候选继续修复？确认前我不会修改 formal pages、log.md、QMD 或 EPI artifacts。",
            "",
        ]
    )
    return "\n".join(lines)
