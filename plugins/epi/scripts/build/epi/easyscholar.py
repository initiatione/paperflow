...
from epi.artifacts import epi_root, utc_now, write_json_atomic


EASYSCHOLAR_API_BASE = "https://www.easyscholar.cc/open"
EASYSCHOLAR_CACHE_SCHEMA = "epi-easyscholar-cache-v1"
EASYSCHOLAR_RECORD_SCHEMA = "epi-easyscholar-record-v1"
...
def _cache_root(vault_path: Path) -> Path:
    return epi_root(vault_path) / "cache" / "easyscholar"
...
def _default_client(publication_name: str, secret_key: str, timeout_seconds: int) -> dict:
    ...
    request = urllib.request.Request(url, headers={"User-Agent": "EPI EasyScholar Enricher"})
