...
Runtime artifacts stay in `_runs`, `_raw`, `_staging`, and the optional Zotero sidecar. The main artifacts are `_runs/<run-id>/search-record.json`, `rank.json`, `report.md`, `report.json`, and `run-state.json`, plus paper-scoped `parse-record.json`, `critic-report.json`, `wiki-ingest-record.json`, and `zotero-record.json` files.
...
The command writes both JSON and Markdown under `.plugin-eval\improvement-briefs\`. Those outputs are local development artifacts and stay out of commits. A complete dev-loop brief must provide all three evidence sources: Plugin Eval JSON, `epi-quality-gates` metric-pack JSON, and a benchmark/comparison JSON. If any source is missing, the brief records `source_completeness.complete=false`, adds the required `quality_loop_sources_complete` gate, and sets `next_action=collect-missing-quality-evidence` instead of treating the brief as ready for a skill-aware-evolve proposal.
