...
- Agent-mediated 完成态记录：新增 `record-wiki-ingest`，在外部 wiki ingest agent 写完最终 Markdown 页后，EPI 只读最终页并记录相对路径、sha256、human approval、`paper-gate` 摘要和 source-first handoff 元数据；该命令不修改最终页、manifest、index、log 或 hot。
