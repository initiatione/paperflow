...
更新时间：2026-06-07。本文档记录当前源码树的开发状态、已完成能力、已知风险和下一步计划。它是面向继续开发和发布验收的进度页，不替代 `docs/epi-linkage.md` 的链路契约，也不替代 `docs/structure.md` 的结构说明。
...
## 2026-06-07 EPI/PRW 健康检查

本轮检查把 source checkout、installed cache 和当前 Codex CLI 可见性分开记录，避免把缓存目录存在误判为当前会话必然可调用。

- Source plugin validation：当前源码收尾使用 repo-local contract tests、`python -m json.tool plugins\epi\.codex-plugin\plugin.json`、`python -m json.tool plugins\PRW\.codex-plugin\plugin.json` 和 `git diff --check`；固定路径 `validate_plugin.py` 在当前 plugin-creator skill 中不存在，只有机器配置了 `PLUGIN_VALIDATE_SCRIPT` 时才额外运行。
- Installed cache validation：installed cache 仍需 marketplace refresh / reinstall 后单独验证；不要把 source checkout 测试结果当作当前会话 live runtime。
- Workflow boundary source version sync：历史 EPI/PRW 工作流边界同步先把 source manifest 从 EPI `0.1.12` / PRW `0.1.5` bump 到 EPI `0.1.13` / PRW `0.1.6`；随后 ask-mode record automation 再 bump 到 EPI `0.1.14` / PRW `0.1.7`。本轮 read-only formal graph `wiki-ask` 新增用户可见问答能力，source manifest 同步到 EPI `0.2.0` / PRW `0.2.0`。installed cache 仍需 marketplace refresh / reinstall 后才会从 EPI `0.1.11`、PRW `0.1.4` 更新。
- Read-only wiki ask：EPI CLI 新增 `wiki-ask --question "<research question>" --vault <vault> [--json]`，从正式页 graph 检索并利用 backlinks/outlinks/aliases/tags/co-links，输出具体的 `【Wiki 证据】`、`【综合判断】`、`【推断】`、`【边界/不确定】` answer sections 和 correction candidates；它只读解析正式页 `sources` 或正文中的 `_epi/raw/<slug>/paper.pdf` source evidence 以确认源 PDF/MinerU artifact 可用性，但 `_epi/raw` 不进入 formal graph；它不写 `log.md`、正式页、QMD、`prw-record-request.json` 或 EPI artifacts，纠错必须 ask before repair。
- PRW -> EPI ask-mode record automation：PRW 正式页和 `final-source-review.json` 完成后写 `_epi/staging/papers/<paper-slug>/prw-record-request.json`；默认 `automation_mode=ask`，当前 agent 或用户确认后由 EPI `record-wiki-ingest --from-prw-request ...` 消费。EPI 重新验证 live page hashes、`final-source-review.json`、human approval、`paper-gate` 和 formal-page rules 后才写 `wiki-ingest-record.json`。
- 聚焦回归：`python -m pytest tests\epi\test_wiki_query.py tests\epi\test_cli_parser.py tests\paper_research_wiki\test_plugin_contract.py tests\epi\test_current_docs.py -q --basetemp=.pytest_tmp_wiki_ask_focused2` 返回 `107 passed in 1.35s`；`python -m pytest tests\paper_research_wiki\test_plugin_contract.py plugins\epi\tests\test_skill_bundle_contract.py tests\test_marketplace_manifest.py -q --basetemp=.pytest_tmp_wiki_ask_contract2` 返回 `55 passed in 0.42s`；`python -m pytest tests\epi\test_wiki_query.py tests\epi\test_cli_parser.py tests\epi\test_current_docs.py -q --basetemp=.pytest_tmp_wiki_ask_epi_docs2` 返回 `70 passed in 1.16s`。
...
- Skill 入口语言：`plugins/epi/skills/*/SKILL.md` 已重构为 English-first 入口说明，并在 frontmatter description 与 `skills/routing.yaml` 中保留真实中文触发短语，例如“找最新论文”“持续跟踪方向”“兼容旧沉淀任务”“优化 EPI 插件”，让用户自然任务先命中正确 route；用户级“沉淀进 wiki”由 PRW `$paper-research-wiki` 承接；`paper-discovery/README.md` 已移除，避免违反 skill-creator 对顶层 README 的精简要求。
- Skill routing contract：`skills/routing.yaml` 现在覆盖每个 `skills/*/SKILL.md`，并把 route 分类为 `primary`、`support` 或 `maintenance`。主任务 skill 面向用户自然请求，support skill 只在主链路需要底层能力或证据边界时进入，maintenance skill 用于清理、评估和受控演化。每个 skill 也提供 `agents/openai.yaml`，用于 UI 展示和默认提示，且默认提示显式包含对应 `$skill-name`。核心入口 `SKILL.md` 只保留触发、边界和 workflow routing，步骤性流程下沉到 `workflows/*.md`，并由 `routing.yaml` 的 `workflows` 字段激活，避免入口重新膨胀成 runbook。
- 精准检索计划：`skills/paper-discovery/scripts/query-planner.py` 可把用户画像/config 和用户主题转换成 `query_plan`，包含 `research_mode`、profile、概念块、`domain_focus_terms`、5-8 条 query variants、source route、recall gap checks 和 quality signals；默认 `dry-run` 已接入 query plan，会写 `query-plan.json`，按 query variants 多次搜索并合并 candidate pool；`query-plan.json` 和 `search-record.json` 现在包含 `source_routing`，默认降级 Google Scholar 等 `unstable` source，并把 Unpaywall email 缺失写成 `provider_gaps` / `unpaywall_email_missing`；`report.json.discovery_context.source_coverage` 和 `report.md` 的 `Source Coverage` / `Source Routing` 现在保留 MCP/CLI 上游的 `sources_used`、`source_results`、`errors`、`raw_total`、`deduped_total`、`query_count`、`capabilities`、`provider_readiness`、`source_routing` 和 `provider_gaps`，用于判断多源召回深度、source gap、source download/read 能力边界和 provider env 缺口；`dry-run --json` 可打印 run id 和关键 artifact 路径，方便 agent 串联；默认排除 review/survey/meta 类综述候选，用户明确找综述时才放开。EPI 不再把 AUV/机器人/AI 等词作为全局默认，而是从 `_epi\meta\epi-config.yaml` 的 profile、domains、positive_keywords、negative_keywords、venue_prior 和当前请求衍生检索词；query plan 的扩展词只用于召回，`domain_focus_terms` 才是 narrow topic 的硬锚点，ranking 的画像匹配词使用用户 config 和当前请求核心词，避免宽召回词稀释窄主题相关性；`rank.json` 现在记录 `paper_type`、`paper_classification`、`quality_gate`、`quality_tier`、`ranking_rubric` 和 `ranking_confidence`，用于解释阅读优先级；若 profile-derived 计划偏离当前请求，使用 `--no-query-plan` 精确短语查询或先更新 config。
- 默认快路：`prepare-ranked` 支持从 dry-run 的 `rank.json` 直接完成 selected ranked paper acquire + MinerU parse + source-staging 报告，并停在 human approval / final wiki writing 之前。默认 `fast-ingest` 不跑 reader/critic；`reviewed-ingest` 才加 reader；`audited-ingest` 才加 reader+critic。实测批量用 `--max-papers 10 --skip-existing`，补跑时只有 `parse-record.json status=success`、MinerU companion artifacts 完整且同模式 `promotion-plan.json` 已存在的论文才不占预算；`--max-papers 1` 只作为 smoke test；`--json` 会输出 prepared run id、source run id、processed/skipped counts、`stops_after=source-staging` 和关键 report artifact 路径，方便 agent 串联。采集阶段优先调用 paper-search MCP `download_with_fallback`，默认开放访问链为 source-native、OpenAIRE、CORE、Europe PMC、PMC、Unpaywall，Sci-Hub 默认关闭；没有 direct PDF URL 且 OA fallback 无 PDF 时快速写 `failure_class=manual-download-required`、`manual_download.candidate_manual_urls` 和 organization/institution 手动下载提示，不继续弱 fallback，且 `prepare-ranked` report / `runs-query --json` 会上浮 `manual_downloads`；成功 `acquire-record.json` 记录 `doi`、`title`、`use_scihub`、`fallback_chain`、`mcp_server_probe`、`upstream.tool` 和 `identity_check`。采集成功后可调用 `read_<source>_paper` 或 CLI read 写 `paper-search-read-preview.txt`，并在 `retrieval_preview` 记录 non-authoritative preview；这用于检查上游 retrieval depth，does not replace MinerU。若 PDF DOI/title 明确不匹配，写 `identity-check.json`、`failure_class=identity-mismatch`，隔离到 `_epi/quarantine/papers/<slug>/paper.pdf`，并阻止 MinerU。单篇下载/解析异常会记录为该论文的失败状态并继续后续候选。
...
- EPI paper deposition adapter：`epi-paper-deposition` skill 作为 EPI source bundle / `wiki_deposition_task.json` 到 obsidian-wiki skill layer 的 compatibility adapter；用户级正式论文 wiki 写入和维护由 PRW `$paper-research-wiki` 作为 canonical 入口，旧 `epi-wiki-deposition` 只保留为 compatibility alias。
- Agent-mediated wiki ingest：默认不由固定脚本写最终 wiki，而是生成 `wiki-ingest-handoff` 和 `wiki_deposition_task.json`，要求 agent 通过 PRW `$paper-research-wiki` 显式使用 `llm-wiki`、`wiki-ingest`、`wiki-context-pack`、`wiki-lint`、`wiki-stage-commit`、`wiki-status`、`wiki-query`、`wiki-provenance`、`tag-taxonomy`，并读取目标 vault contract 后再写入；`epi-paper-deposition` 只保留旧 handoff 和 record provenance 的兼容桥。
...
- Zotero record-only sidecar：`record-wiki-ingest` 和 legacy `promote-to-wiki` 会根据 vault config 写 `_epi/raw/<slug>/zotero-record.json`，记录 skipped/recorded 状态、collection、论文 metadata、wiki ingest 摘要和 final page hashes；结果同步进入 `report.json`、`run-state.json`、`_epi/runs/index.json` 和 dashboard，不调用外部 Zotero API。
...
- 查询与维护：`runs-query`、`research-queue`、legacy `wiki-query` 和 read-only `wiki-ask` 支持状态/知识查询；redo/recritic 支持阶段性修复，audited-ingest 不绕过 critic gate。
- 过渡态生命周期：`run-lifecycle` 支持 `_epi/runs` 单次 run 目录 dry-run/apply 清理，保留面板文件并写 `_epi/meta/run-lifecycle` manifest；EPI run 结束后会自动检查 `_epi/runs`，超过 15 个单次 run 目录时自动应用 cleanup，保留最新 15 个 terminal run 并按 workflow 至少保留 2 个；`epi-repository-cleanup` 会按 `_epi/policies/retention.json` 检查内部仓库总文件数/体积，并且即使未超总量也会执行 lifecycle 上限：清理旧 terminal run dirs、`run-lifecycle`、`raw-cleanup`、`repository-maintenance`、`migrations`、`wiki-reset`、`formal-page-snapshots` 和 `tmp-manual-pdfs`，默认不删除 `_epi/raw` 原始论文源；`--preview --json` 是 no-write inspection。发现阶段会和 `_epi/raw` 已下载论文按 DOI/arXiv/title 去重，避免重复推荐。

## 本轮相关变更范围

本轮 paper-search MCP P0 hardening 范围是：在 `0.1.10` 的 provider/source-capability 基础上，把检索路由、无 PDF 时的人工补 PDF 入口、以及下载后 PDF 身份核验变成一组用户可见契约。插件版本同步到 `0.1.11`，源码验证通过后仍需单独刷新 installed cache 才能成为 live runtime。`0.1.11` 新增 provider-readiness-aware source routing：Google Scholar、BASE、SSRN 等 unstable source 默认降级，Unpaywall email 缺失写成 `provider_gaps` / `unpaywall_email_missing`，不能把 OA fallback 弱解释成论文不可得。采集阶段若没有 direct PDF URL 且 OA fallback 无 PDF，会快速写 `failure_class=manual-download-required`、`manual_download.candidate_manual_urls`，并在 `prepare-ranked` report / `runs-query --json` 上浮 `manual_downloads`，让对话层直接给用户 DOI/出版商链接。任何下载到的 PDF 都先做轻量 DOI/title identity check；明确不匹配时写 `identity-check.json`、`failure_class=identity-mismatch`，把 PDF 隔离到 `_epi/quarantine/papers/<slug>/paper.pdf`，并阻止 MinerU。

- `plugins/epi/.codex-plugin/plugin.json`
- `plugins/epi/.mcp.json`
- `plugins/epi/scripts/build/epi/paper_search_adapter.py`
- `plugins/epi/scripts/build/epi/acquire_papers.py`
...
- `plugins/epi/scripts/build/epi/raw_cleanup.py`
- `plugins/epi/scripts/build/epi/cli.py`
- `plugins/epi/scripts/build/epi/cli_parser.py`
...
- `plugins/epi/docs/attribution.md`
- `plugins/epi/skills/paper-discovery/references/search-protocol.md`
- `plugins/epi/skills/paper-ingest/workflows/prepare-ranked.md`
- `tests/epi/test_paper_search_adapter.py`
- `tests/epi/test_acquire_url.py`
- `tests/epi/test_batch_advance_router.py`
- `tests/epi/test_orchestrator_dry_run.py`
- `tests/epi/test_report_run.py`
- `tests/epi/test_runs_query_cli.py`
- `tests/epi/test_current_docs.py`

本轮 EPI wiki lifecycle 优化范围是：保留 `_meta` 作为正式 wiki contract，继续让 `_epi` 承接 EPI 内部运行仓库，但把 `_epi` 里的短期/维护/临时文件改成有明确保留上限。插件版本同步到 `0.1.8`，源码验证通过后仍需单独刷新 installed cache 才能成为 live runtime。`0.1.8` 在 `0.1.7` 的 paper-search MCP 深度使用基础上新增 bounded lifecycle cleanup：`epi-repository-cleanup --preview --json` 不再写 vault；非 preview cleanup 即使 `over_budget=false` 也会按 retention policy 清理旧 run dirs、维护 manifest、formal-page snapshots 和临时 manual PDFs。
...
- `plugins/epi/docs/structure.md`
- `plugins/epi/docs/attribution.md`
- `plugins/epi/docs/privacy.md`
- `plugins/epi/docs/terms.md`
- `plugins/epi/skills/paper-discovery/references/search-protocol.md`
- `plugins/epi/skills/paper-ingest/workflows/prepare-ranked.md`
- `plugins/epi/vendor-notices/paper-search-mcp.md`
- `tests/epi/test_paper_search_adapter.py`
- `tests/epi/test_acquire_url.py`
- `tests/epi/test_orchestrator_dry_run.py`
- `tests/epi/test_report_run.py`
...
本轮 skill-based-architecture / skill-creator 对齐范围是：把已拆分出来的 11 个 EPI skills 全部接入插件级 routing，按用户任务链标记 `primary/support/maintenance`，补充中文自然触发短语和 UI metadata；同时把 `paper-discovery`、`paper-ingest`、`epi-paper-deposition`、`wiki-setup` 的步骤层拆到 `workflows/`，让 `SKILL.md` 保持短入口。新增 bundle contract 防止以后新增 skill 后漏接 routing、workflow、metadata 或带入 Python cache 生成物。

- `plugins/epi/skills/routing.yaml`
- `plugins/epi/skills/*/SKILL.md`
- `plugins/epi/skills/*/agents/openai.yaml`
- `plugins/epi/skills/*/workflows/*.md`
- `plugins/epi/tests/test_skill_bundle_contract.py`
- `plugins/epi/docs/workflow.md`
- `plugins/epi/docs/progress.md`
...
本轮 paper-search MCP source coverage / OA fallback / retrieval preview 优化已验证：

```powershell
python -m pytest tests\epi\test_paper_search_adapter.py tests\epi\test_acquire_url.py tests\epi\test_report_run.py tests\epi\test_orchestrator_dry_run.py tests\epi\test_current_docs.py -q --basetemp=.pytest_tmp_epi_mcp_read_preview_focused
python -m pytest tests\paper_research_wiki\test_plugin_contract.py plugins\epi\tests\test_skill_bundle_contract.py -q --basetemp=.pytest_tmp_epi_mcp_read_preview_contracts
python -m pytest tests\epi -q --basetemp=.pytest_tmp_epi_mcp_read_preview_full2
python <plugin-creator-validate-script> <plugin-root>\plugins\epi
python <plugin-creator-validate-script> <plugin-root>\plugins\PRW
git diff --check
```

结果：MCP/read-preview focused 回归为 `63 passed in 14.16s`；PRW/EPI contract 回归为 `40 passed in 0.47s`；审查后补测 source coverage dedupe、info-only read preview 过滤和 fallback success probe 一致性为 `3 passed in 2.96s`；全量 `tests\epi` 为 `391 passed in 53.81s`；`plugins\epi\tests` 为 `12 passed in 0.44s`；EPI plugin validation passed；PRW plugin validation passed；`git diff --check` 无 whitespace error，仅有 Windows LF/CRLF 提示。当前 `plugin.json` 版本为 `0.1.8`，installed cache 仍需单独 refresh/reinstall 才代表 live runtime。

本轮 PRW/EPI canonical routing 与 line-count cleanup 已验证：

```powershell
python -m pytest tests\paper_research_wiki\test_plugin_contract.py plugins\epi\tests\test_skill_bundle_contract.py -q
python -m pytest tests\epi\test_wiki_deposition_task.py tests\epi\test_wiki_ingest_handoff.py -q
```

结果：PRW/EPI contract 聚焦回归为 `40 passed`；EPI handoff/deposition sanity 回归为 `9 passed`。当前 `paper-discovery`、`paper-ingest`、`epi-paper-deposition`、`wiki-setup`、PRW `paper-research-wiki` 的 `SKILL.md` 总行数分别为 60、55、54、72、67；扣除 frontmatter 后的正文行数分别为 53、48、47、65、56。

本轮 skill routing / metadata / workflow-layer 优化已验证：

```powershell
python -m pytest plugins\epi\tests\test_skill_bundle_contract.py tests\epi\test_config_onboarding_docs.py::test_wiki_setup_skill_owns_vault_initialization_and_reset_confirmation tests\epi\test_config_onboarding_docs.py::test_epi_skills_document_precise_one_to_three_prepare_ranked_path tests\epi\test_config_onboarding_docs.py::test_paper_discovery_skill_documents_quality_first_chat_recommendations tests\epi\test_config_onboarding_docs.py::test_paper_discovery_skill_defines_stronger_high_quality_search_protocol tests\epi\test_current_docs.py::test_epi_skill_architecture_alignment_documents_routing_and_closure tests\epi\test_current_docs.py::test_human_approval_requires_single_readable_approval_report -q --basetemp .pytest_tmp_workflow_split_final_relevant2
python <skill-creator-quick-validate-script> <plugin-root>\skills\<skill-name>
```

结果：workflow-layer 相关聚焦回归为 `14 passed in 0.50s`；11 个 skill quick validation 均为 `Skill is valid!`。当前源码树中 `paper-discovery`、`paper-ingest`、`epi-paper-deposition`、`wiki-setup`、PRW `paper-research-wiki` 的 `SKILL.md` 总行数分别为 60、55、54、72、67；扣除 frontmatter 后的正文行数分别为 53、48、47、65、56，并由 `test_core_skill_entrypoints_route_to_workflows_without_becoming_runbooks` 和 PRW entrypoint contract tests 防止入口重新膨胀、`workflows/*.md` 不孤儿化。本轮还新增了 tracked Python cache artifact gate，并清理了 skill bundle 内测试生成的 `__pycache__` 目录。
