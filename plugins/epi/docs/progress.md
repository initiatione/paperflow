...
- Skill 入口语言：`plugins/epi/skills/*/SKILL.md` 已重构为 English-first 入口说明，并在 frontmatter description 与 `skills/routing.yaml` 中保留真实中文触发短语，例如“找最新论文”“持续跟踪方向”“沉淀进 wiki”“优化 EPI 插件”，让用户自然任务先命中正确 route；`paper-discovery/README.md` 已移除，避免违反 skill-creator 对顶层 README 的精简要求。
...
结果：workflow-layer 相关聚焦回归为 `14 passed in 0.50s`；11 个 skill quick validation 均为 `Skill is valid!`。`paper-discovery`、`paper-ingest`、`epi-paper-deposition`、`wiki-setup` 的入口分别约束在 60、53、47、70 行，并由 `test_core_skill_entrypoints_route_to_workflows_without_becoming_runbooks` 检查入口不重新膨胀、`workflows/*.md` 不孤儿化。本轮还新增了 tracked Python cache artifact gate，并清理了 skill bundle 内测试生成的 `__pycache__` 目录。
