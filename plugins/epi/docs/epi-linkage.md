...
- 留痕：保存 PDF、元数据、MinerU Markdown/TeX/images/manifest、可选 reader/critic、source-staging handoff 和 run-state。
...
- `_epi/raw/papers/<slug>/`：PDF、metadata、MinerU Markdown/TeX/images/manifest、reader、critic 和论文级 run-state。
...
- `_epi/meta/`：EPI config、config history、run lifecycle、migration 和 repository maintenance manifests。
- `_epi/policies/retention.json`：自动清理阈值；默认只清 terminal run/staging 类可再生产物，不自动删除原始论文源文件。
...
- `_epi/raw/papers/<slug>/run-state.json`
...
`_epi/runs` 是过渡态证据区，需要生命周期管理。EPI run 结束后会自动检查 `_epi/runs` 下单次 run 目录数量；超过 15 个时自动应用 lifecycle cleanup，保留最新 15 个 terminal run，并按 workflow 至少保留 2 个。随后还会按 `_epi/policies/retention.json` 检查整个 `_epi` 仓库的文件数和体积阈值，超额时优先清理 terminal run 目录等可再生产物。手动入口：`run-lifecycle --keep-latest 15 --keep-per-workflow 2 [--max-age-days N] [--apply] [--json]` 和 `epi-repository-cleanup --preview --json` / `epi-repository-cleanup --json`。手动 run lifecycle 默认 dry-run，只列候选；`--apply` 才删除单次 run 目录并刷新 index/dashboard/research-queue。它不得删除 `_epi/raw/papers` 原始论文、最终 wiki、Zotero 记录或配置历史；清理 manifest 写入 `_epi/meta/run-lifecycle/` 或 `_epi/meta/repository-maintenance/`。

论文发现必须和已下载库去重。`dry-run` 在 filter 阶段扫描 `_epi/raw/papers/*/metadata.json`，按 DOI、arXiv ID、normalized title 匹配已入库论文；命中时写入 `filter_reasons: already_in_library:<slug>`，不得再次进入 `rank.json` 推荐。
