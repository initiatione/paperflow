...
`_epi/README.md` 是给 AI/agent 的导航入口，`_epi/manifest.json` 是机器可读目录和统计索引，`_epi/policies/retention.json` 是仓库清理策略。Obsidian graph 应忽略 `_epi/`；正式图谱页只应由 wiki skill 写到 `references/`、`concepts/`、`synthesis/`、`reports/` 等正式目录。物理结构：

- `_epi/raw/papers/<slug>/`：PDF、metadata、MinerU Markdown/TeX/images/manifest、reader、critic 和论文级 run-state。
- `_epi/staging/papers/<slug>/`：source-reader、reading-report、wiki-ingest-brief、human approval、trigger、final-source-review 和 record sidecars。
...
当前默认路径是 wiki-skill batch deposition handoff：`promotion-plan.json` 记录 `handoff_type=agent-mediated-wiki-ingest`、`wiki_write_model=wiki-skill-batch-distillation`、`final_page_authority=wiki-skill-batch-distillation`、`wiki_ingest_brief_path`、`wiki_batch_ingest_brief_path`、`agent_handoff_paths`、`final_source_review_contract` 和 `suggested_final_source_review_path`。EPI 不用固定脚本决定最终 Obsidian Wiki 页面路径；`references/`、`concepts/`、`synthesis/`、`reports/` 是 wiki skill 的正式写入区，不再由 EPI staging 生成 per-paper pseudo routes。
