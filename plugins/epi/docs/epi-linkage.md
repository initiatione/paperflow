...
当前默认路径是 wiki-skill batch deposition handoff：`promotion-plan.json` 记录 `handoff_type=agent-mediated-wiki-ingest`、`wiki_write_model=wiki-skill-batch-distillation`、`final_page_authority=wiki-skill-batch-distillation`、`wiki_ingest_brief_path`、`wiki_batch_ingest_brief_path`、`wiki_deposition_task_path`、`agent_handoff_paths`、`final_source_review_contract` 和 `suggested_final_source_review_path`。EPI 不用固定脚本决定最终 Obsidian Wiki 页面路径；`references/`、`concepts/`、`derivations/`、`experiments/`、`synthesis/`、`reports/`、`opportunities/` 是 `epi-paper-deposition` 和 obsidian-wiki skill layer 的正式写入区，不再由 EPI staging 生成 per-paper pseudo routes。`epi-wiki-deposition` 是旧 artifact 兼容名。
...
用户给出方向后，EPI 找候选论文并排序；用户或批处理推进少量高价值论文；EPI 保存原始证据并生成多角色 reader；critic 检查身份、证据、benchmark、scope 和解析质量；通过后生成 evidence staging、wiki-ingest brief、`wiki_deposition_task.json` 和轻阅读报告；用户批准后由 agent 通过 `epi-paper-deposition` 按目标 vault contract 写入 Obsidian/LLM Wiki staging/formal pages；写入完成后用 `record-wiki-ingest` 回填最终页路径、hash 和 approval 完成态；用户从阅读报告开始低负担吸收知识。
