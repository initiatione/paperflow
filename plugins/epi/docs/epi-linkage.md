...
- `_epi/raw/papers/<slug>/mineru/paper.md`
...
入口：`record-human-approval --slug <slug> --approved-by <name> --scope run-wiki-ingest-agent [--json]`。它是 agent-mediated wiki ingest 的前置人类批准记录器，只在当前 `paper-gate` 无 failure checks 且唯一 action-required 为 `human-approval` 时写 `_epi/staging/papers/<slug>/human-approval.json`，schema 为 `epi-human-approval-v1`。它不得写最终 wiki 页面，也不得替代 final wiki agent。
