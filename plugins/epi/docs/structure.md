...
EPI 是 Codex 插件源码包，位于 `D:\paper-search\plugins\epi`。插件的目标链路是：高质量论文发现和排序 -> raw artifact 留痕 -> Reader/Critic 证据审查 -> staging evidence package -> wiki-ingest handoff -> 由 agent 按目标 Obsidian/LLM Wiki vault contract 写入最终知识库。
...
源码测试主要在 `D:\paper-search\tests\epi`。插件内 wrapper smoke 由源码测试覆盖，避免在插件包内放重复测试文件增加 token budget。
