...
- `paper-discovery`：搜索、排序和 dry-run；当用户要求“1-3”时，指向 `prepare-ranked` 快路，只写 raw 采集和 MinerU 解析产物。
  - `paper-discovery/README.md` 与 `references/` 保存可维护检索策略：source tiers、dedup engine、venue prior、multi-source workflow、quality gate 和对话输出格式。RoboWiki/社区榜单只进入 `venue_prior`，真实指标仍需单独核验。
