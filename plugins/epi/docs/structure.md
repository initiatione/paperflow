...
- `epi-paper-deposition`：EPI source bundle 与 obsidian-wiki skill layer 的正式 adapter；读取 `wiki_deposition_task.json`，要求 `llm-wiki`、`wiki-ingest`、`wiki-context-pack`、`wiki-lint`、`wiki-stage-commit`、`wiki-status`、`wiki-query`、`wiki-provenance` 和 `tag-taxonomy`，保留七类 EPI 目录，并把旧 `epi-wiki-deposition` 只当兼容 alias。
