...
9. When a request activates an explicit domain hint pack, use that pack's strong synonyms and the user's configured domain terms as the hard gate. Do not promote broad n-grams sliced from a long query, such as generic vehicle/task phrases, into `domain_focus_terms` if they would let adjacent but off-domain papers pass without the requested object/domain anchor.
