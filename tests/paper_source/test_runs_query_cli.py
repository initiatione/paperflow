...
def _seed_ready_paper_gate(vault, slug):
    ...
    batch_path = vault / "_paper_source/staging/wiki-batches/pending/wiki-batch-ingest-brief.json"
...
def test_report_cli_prints_existing_run_markdown(tmp_path, monkeypatch, capsys):
    ...
    runs_root = vault / "_paper_source/runs"
    run_dir = _seed_run(
        runs_root,
...
def test_report_cli_json_allows_markdown_only_report(tmp_path, monkeypatch, capsys):
    vault = tmp_path / "vault"
    run_dir = vault / "_paper_source/runs" / "20260528T123000Z-report"
    run_dir.mkdir(parents=True, exist_ok=True)
...
def test_report_loader_errors_when_run_dir_missing(tmp_path):
    with pytest.raises(FileNotFoundError, match="missing Paper Source run directory"):
        load_run_report(tmp_path / "vault", "missing-run")


def test_report_loader_errors_when_report_artifacts_missing(tmp_path):
    vault = tmp_path / "vault"
    (vault / "_paper_source/runs" / "empty-report-run").mkdir(parents=True)
...
def test_research_queue_cli_filters_bucket_and_shows_checks(tmp_path, monkeypatch, capsys):
    ...
    assert exit_code == 0
    assert "Paper Source Research Queue - reproducibility_caveats" in output
            ...
            "reader_revision_plans": [
                {
                    ...
                    "blocking_count": 2,
                    "warning_count": 1,
                    "plan_path": str(vault / "_paper_source/raw" / "repair-paper" / "critic" / "reader-revision-plan.json"),
                }
            ],
        },
    )
...
def test_research_agenda_cli_is_not_available(tmp_path, monkeypatch, capsys):
    ...
    try:
        ...
        raise AssertionError("research-agenda should not be available after narrowing Paper Source scope")
