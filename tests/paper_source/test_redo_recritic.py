...
from paper_source.orchestrator import main, run_one_paper_ingest
from paper_source.redo import redo_acquire, redo_parse, redo_read, recritic
...
def _single_run_dir(vault):
    run_dirs = [path for path in (vault / "_paper_source" / "runs").iterdir() if path.is_dir()]
...
def test_redo_read_consumes_reader_revision_plan_as_role_guidance(tmp_path):
    ...
    plan_path.write_text(
        json.dumps(
            {
                "schema_version": "paper-source-reader-revision-plan-v1",
