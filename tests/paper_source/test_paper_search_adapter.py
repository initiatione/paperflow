...
from paper_source.paper_search_adapter import download_paper_pdf
from paper_source.paper_search_adapter import discover
...
import paper_source.paper_search_adapter as paper_search_adapter
...
def test_mcp_jsonrpc_client_info_uses_paper_source_name(monkeypatch):
    messages = []
    ...
    def _capture_write(process, payload):
        messages.append(payload)

    def _fake_read(*, expected_id, **kwargs):
        if expected_id == 1:
            return {"result": {"protocolVersion": "2024-11-05", "serverInfo": {"name": "fake"}}}
        return {"result": {"structuredContent": {"ok": True}}}

    monkeypatch.setattr(paper_search_adapter.subprocess, "Popen", lambda *args, **kwargs: _FakeProcess())
    monkeypatch.setattr(paper_search_adapter.threading, "Thread", lambda *args, **kwargs: type("T", (), {"start": lambda self: None})())
    monkeypatch.setattr(paper_search_adapter, "_write_jsonrpc", _capture_write)
    monkeypatch.setattr(paper_search_adapter, "_read_jsonrpc_response", _fake_read)
    monkeypatch.setattr(paper_search_adapter, "_extract_mcp_tool_payload", lambda result: {"ok": True})
    monkeypatch.setattr(paper_search_adapter, "_close_process_stdin", lambda process: None)

    result = paper_search_adapter._call_mcp_tool("search_papers", {"query": "auv"}, timeout_seconds=5)
...
def test_mcp_probe_client_info_uses_paper_source_doctor_name(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: {"result": {"protocolVersion": "2024-11-05", "serverInfo": {"name": "fake"}}},
    )
    monkeypatch.setattr(paper_search_adapter, "_close_process_stdin", lambda process: None)

    monkeypatch.setattr(
        paper_search_adapter,
        "_paper_search_mcp_disabled",
        lambda: False,
    )
    monkeypatch.setattr(
        paper_search_adapter,
        "_mcp_command_args",
        lambda: (["python", "-m", "fake"], {"command": "python", "args": ["-m", "fake"], "transport": "stdio"}),
    )

    result = paper_search_adapter.probe_paper_search_mcp_server(timeout_seconds=5)

    assert result["available"] is True
    assert messages[0]["params"]["clientInfo"]["name"] == "paper-source-doctor"
...
def test_discovery_prefers_paper_search_mcp_server_over_cli(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("paper_source.paper_search_adapter._call_mcp_tool", _fake_mcp_tool)
    monkeypatch.setattr("paper_source.paper_search_adapter._run_command", _unexpected_cli)
...
def test_discovery_records_source_health_for_mcp_search(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("paper_source.paper_search_adapter._call_mcp_tool", _fake_mcp_tool)
    monkeypatch.setattr("paper_source.paper_search_adapter.time.perf_counter", lambda: next(perf_counter_values))
...
def test_discovery_falls_back_to_cli_when_paper_search_mcp_server_fails(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("paper_source.paper_search_adapter._call_mcp_tool", _failing_mcp_tool)
...
def test_discovery_falls_back_to_cli_when_mcp_returns_no_papers(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("paper_source.paper_search_adapter._call_mcp_tool", _empty_mcp_tool)
...
def test_download_treats_cli_timeout_as_upstream_failure(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("paper_source.paper_search_adapter._run_command", _run_command)
...
def test_read_preview_rejects_info_only_mcp_text(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("paper_source.paper_search_adapter._call_mcp_tool", _info_only_mcp_tool)
...
def test_probe_paper_search_fails_closed_on_timeout(monkeypatch):
    ...
    monkeypatch.setattr("paper_source.paper_search_adapter._run_command", _timeout)
