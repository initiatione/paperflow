...
def test_wiki_ingest_handoff_exposes_evidence_index_locator(tmp_path):
    ...
    evidence["vault_evidence_index"] = "_paper_source/meta/evidence-index.json"
...
def test_wiki_ingest_trigger_writes_agent_neutral_trigger_after_approval(tmp_path):
    ...
    trigger = build_wiki_ingest_trigger(vault, slug)
    trigger_path = vault / "_paper_source" / "staging" / "papers" / slug / "wiki-agent-trigger.json"
    stored = json.loads(trigger_path.read_text(encoding="utf-8"))

    assert trigger["schema_version"] == "paper-source-wiki-agent-trigger-v1"
    ...
    assert stored["schema_version"] == "paper-source-wiki-agent-trigger-v1"
...
def test_render_wiki_ingest_trigger_shows_resume_command_after_approval(tmp_path):
    ...
    assert "# Paper Source Wiki Agent Trigger - fixture-paper" in output
...
def test_render_wiki_ingest_handoff_is_actionable_without_writing(tmp_path):
    ...
    assert "# Paper Source Wiki Ingest Handoff - fixture-paper" in output
    ...
    assert handoff["required_wiki_skills"] == EXPECTED_RESEARCH_WIKI_SKILLS
    assert "Do not write final pages from Paper Source suggested routes directly" in output
