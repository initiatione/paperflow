...
def test_paper_gate_reports_ready_paper_waiting_for_human_approval(tmp_path):
    ...
    assert "# Paper Source Paper Gate - fixture-paper" in rendered
...
def test_paper_gate_surfaces_corrected_premature_wiki_ingest_record(tmp_path):
    ...
    correction_path = vault / "_paper_source" / "meta" / "record-corrections" / "20260606-premature.json"
    _write_json(
        correction_path,
        {
            ...
            "action_taken": {
                "staging_location": "_paper_source/staging/premature-formal-pages/20260606-fixture",
            },
            "status_after_correction": {
                "wiki_quality_status": "pending-paper-wiki-review",
    ...
    assert gate["check_suite"]["conclusion"] == "action_required"
    assert gate["next_action"] == "run-paper-wiki-review"
    checks = {run["name"]: run for run in gate["check_suite"]["check_runs"]}
    assert checks["human-approval"]["conclusion"] == "success"
    ...
    assert checks["record-correction"]["details"]["wiki_quality_status"] == "pending-paper-wiki-review"
    assert (
        checks["record-correction"]["details"]["staging_location"]
        == "_paper_source/staging/premature-formal-pages/20260606-fixture"
    )
    assert (
