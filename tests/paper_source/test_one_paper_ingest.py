...
from paper_source.orchestrator import run_one_paper_ingest
...
def test_one_paper_ingest_preserves_raw_artifacts_and_stages_after_critic_pass(tmp_path):
    ...
    assert "- Paper Source 只写 `_paper_source/` 内部材料；正式图谱页由 wiki skill 批量沉淀生成。" in reading_report_text
    ...
    assert wiki_ingest_brief["schema_version"] == "paper-source-wiki-ingest-brief-v1"
    ...
    assert wiki_ingest_brief["ingest_policy"]["paper_source_write_scope"] == "internal-underscore-artifacts-only"
    ...
    assert final_source_review_contract["schema_version"] == "paper-source-final-source-review-contract-v1"
    ...
    assert promotion_plan["paper_source_write_scope"] == "internal-underscore-artifacts-only"
    ...
    batch_handoff_path = tmp_path / "vault" / "_paper_source" / "staging" / "wiki-batches" / "pending" / "wiki-batch-ingest-brief.json"
    ...
    assert batch_handoff["schema_version"] == "paper-source-wiki-batch-ingest-brief-v1"
    assert slug in batch_handoff["paper_slugs"]
    assert batch_handoff["paper_source_write_scope"] == "internal-underscore-artifacts-only"
    ...
    assert 'paper_source_recommendation: "stage-for-promotion-review"' in source_reader
    assert 'paper_source_panel_consensus: "approve-for-staging"' in source_reader
    assert 'paper_source_nature_sci_editor_verdict: "pass"' in source_reader
    assert 'paper_source_peer_reviewer_verdict: "pass"' in source_reader
    assert 'paper_source_senior_domain_researcher_verdict: "pass"' in source_reader
    assert "paper_source_blocking_lenses: []" in source_reader
    assert "paper_source_warning_reviewers:" in source_reader
...
def test_stage_paper_rejects_nonpassing_critic(tmp_path):
    paper_root = tmp_path / "_paper_source" / "raw" / "paper"
    ...
    assert not (tmp_path / "_paper_source" / "staging" / "papers" / "paper").exists()
...
def test_critic_quorum_records_reviewer_failure_without_staging(tmp_path):
    ...
    assert not (vault / "_paper_source" / "staging" / "papers" / slug).exists()


def test_critic_quorum_records_missing_reader_as_reviewer_failure(tmp_path):
    slug = "missing-reader-paper"
    paper_root = tmp_path / "_paper_source" / "raw" / slug
    (paper_root / "mineru").mkdir(parents=True)
    (paper_root / "paper.pdf").write_bytes(b"%PDF-1.4\n")
    (paper_root / "metadata.json").write_text(
        json.dumps({"slug": slug, "title": "Fixture Paper", "doi": "10.1000/fixture", "venue": "IROS"}),
